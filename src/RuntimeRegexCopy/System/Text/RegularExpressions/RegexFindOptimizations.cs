...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public sealed class RegexFindOptimizations
    {
        ...
        public RegexFindOptimizations(RegexNode root, RegexOptions options)
        {
            ...
            if (!_rightToLeft) // haven't added FindNextStartingPositionMode trailing anchor support for RTL
            {
                ...
                if (TrailingAnchor is RegexNodeKind.End or RegexNodeKind.EndZ && root.ComputeMaxLength()is int maxLength)
                {
            ...
            if (_rightToLeft)
            {
                ...
                if (RegexPrefixAnalyzer.FindFirstCharClass(root)is string charClass)
                {
                    ...
                    int scratchCount;
                    char[]? chars = null;
                    if (!RegexCharClass.IsNegated(charClass) && (scratchCount = RegexCharClass.GetSetChars(charClass, scratch)) > 0)
                    {
            ...
            if (fixedDistanceSets is not null && FindFixedDistanceString(fixedDistanceSets)is (string String, int Distance) bestFixedDistanceString)
            {
            ...
            if (fixedDistanceSets is not null)
            {
                ...
                if (fixedDistanceSets[0].Chars is not null || literalAfterLoop is null)
                {
                    ...
                    if (!compiled && fixedDistanceSets.Count == 1 && fixedDistanceSets[0].Chars is { Length: 1 })
                    {
                    ...
                    else
                    {
                        ...
                        FindMode = (fixedDistanceSets.Count == 1 && fixedDistanceSets[0].Distance == 0) ? FindNextStartingPositionMode.LeadingSet_LeftToRight : FindNextStartingPositionMode.FixedDistanceSets_LeftToRight;
        ...
        public bool IsUseful => FindMode != FindNextStartingPositionMode.NoSearch || // there's a searching scheme available
 LeadingAnchor == RegexNodeKind.Bol; // there's a leading BOL anchor we can otherwise search for
            ...
            switch (FindMode)
            {
                ...
                case FindNextStartingPositionMode.LiteralAfterLoop_LeftToRight:
                {
                    ...
                    while (true)
                    {
                        ...
                        int i = literal.String is not null ? slice.IndexOf(literal.String.AsSpan()) : literal.Chars is not null ? slice.IndexOfAny(literal.Chars.AsSpan()) : slice.IndexOf(literal.Char);
                        if (i < 0)
                        {
                            break;
                        }
    ...
    public enum FindNextStartingPositionMode
    {
