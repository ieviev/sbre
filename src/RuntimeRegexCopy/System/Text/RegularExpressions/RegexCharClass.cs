...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public sealed partial class RegexCharClass
    {
        // Constants
 ...
 {"L", "\u0000\u0002\u0004\u0005\u0003\u0001\u0000"}, // InternalRegexIgnoreCase = {LowercaseLetter} OR {TitlecaseLetter} OR {UppercaseLetter}
 // !!!This category should only ever be used in conjunction with RegexOptions.IgnoreCase code paths!!!
        {InternalRegexIgnoreCase, "\u0000\u0002\u0003\u0001\u0000"}, // Marks
 ...
 {"Z", "\u0000\u000D\u000E\u000C\u0000"}, };
        ...
        **/
        ...
        private StringBuilder EnsureCategories() => _categories ??= new StringBuilder();
        private List<(char First, char Last)> EnsureRangeList() => _rangelist ??= new List<(char First, char Last)>(6);
        /// <summary>
        ...
        /// </summary>
        public void AddRange(char first, char last) => EnsureRangeList().Add((first, last));
        ...
        public static bool IsMergeable(string charClass) => charClass != null && !IsNegated(charClass) && !IsSubtraction(charClass);
        public static bool IsEmpty(string charClass) => charClass[CategoryLengthIndex] == 0 && charClass[SetLengthIndex] == 0 && !IsNegated(charClass) && !IsSubtraction(charClass);
        ...
        /// </remarks>
        public static bool IsSingleton(string set) => set[CategoryLengthIndex] == 0 && set[SetLengthIndex] == 2 && !IsNegated(set) && !IsSubtraction(set) && (set[SetStartIndex] == LastChar || set[SetStartIndex] + 1 == set[SetStartIndex + 1]);
        public static bool IsSingletonInverse(string set) => set[CategoryLengthIndex] == 0 && set[SetLengthIndex] == 2 && IsNegated(set) && !IsSubtraction(set) && (set[SetStartIndex] == LastChar || set[SetStartIndex] + 1 == set[SetStartIndex + 1]);
        /// <summary>
        ...
        public static bool TryGetOnlyCategories(string set, Span<UnicodeCategory> categories, out int numCategories, out bool negated)
        {
            ...
            for (int pos = SetStartIndex; pos < categoryEnd; pos++)
            {
                ...
                if (c > 0)
                {
                    ...
                    if ((sawFirstCategory && negated) || c == SpaceConst || numCategories == categories.Length)
                    {
                        return false;
                    }
                ...
                else if (c < 0)
                {
                    ...
                    // not a real UnicodeCategory.
                    if ((sawFirstCategory && !negated) || c == NotSpaceConst || numCategories == categories.Length)
                    {
                        return false;
                    }
 ...
 set.Length == SetStartIndex + set[SetLengthIndex]) // and no subtraction
        ...
        public static bool MayOverlap(string set1, string set2)
        {
            ...
            return true;
            static bool KnownDistinctSets(string set1, string set2) => (set1 == SpaceClass || set1 == ECMASpaceClass) && (set2 == DigitClass || set2 == WordClass || set2 == ECMADigitClass || set2 == ECMAWordClass);
        ...
        private static bool CanEasilyEnumerateSetContents(string set) => CanEasilyEnumerateSetContents(set, out bool hasSubtraction) && !hasSubtraction;
        /// <summary>Gets whether we can iterate through the set list pairs in order to completely enumerate the set's contents.</summary>
        ...
        private static bool CanEasilyEnumerateSetContents(string set, out bool hasSubtraction)
        {
            ...
            return set.Length > SetStartIndex && set[SetLengthIndex] > 0 && set[SetLengthIndex] % 2 == 0 && set[CategoryLengthIndex] == 0;
        }

        /// <summary>Provides results from <see cref = "Analyze"/>.</summary>
        public struct CharClassAnalysisResults
        {
        ...
        public static CharClassAnalysisResults Analyze(string set)
        {
            ...
            if (!CanEasilyEnumerateSetContents(set, out bool hasSubtraction) || (isNegated && hasSubtraction))
            {
            ...
            if (isNegated)
            {
                ...
                return new CharClassAnalysisResults{OnlyRanges = true, AllNonAsciiContained = lastValueExclusive <= 128, AllAsciiContained = firstValueInclusive >= 128, ContainsNoAscii = firstValueInclusive == 0 && set[SetStartIndex + 1] >= 128, ContainsOnlyAscii = false, LowerBoundInclusiveIfOnlyRanges = firstValueInclusive, UpperBoundExclusiveIfOnlyRanges = lastValueExclusive, };
            }
            ...
            return new CharClassAnalysisResults{OnlyRanges = true, AllNonAsciiContained = false, AllAsciiContained = firstValueInclusive == 0 && set[SetStartIndex + 1] >= 128 && !hasSubtraction, ContainsOnlyAscii = lastValueExclusive <= 128, ContainsNoAscii = firstValueInclusive >= 128, LowerBoundInclusiveIfOnlyRanges = firstValueInclusive, UpperBoundExclusiveIfOnlyRanges = lastValueExclusive, };
        }

        public static bool IsSubtraction(string charClass) => charClass.Length > SetStartIndex + charClass[CategoryLengthIndex] + charClass[SetLengthIndex];
        public static bool IsNegated(string set) => set[FlagsIndex] == 1;
        public static bool IsNegated(string set, int setOffset) => set[FlagsIndex + setOffset] == 1;
        public static bool IsECMAWordChar(char ch) => // According to ECMA-262, \s, \S, ., ^, and $ use Unicode-based interpretations of
 ...
 ch == '\u0130'; // latin capital letter I with dot above
        ...
        private static ReadOnlySpan<byte> WordCharAsciiLookup => new byte[]{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x03, 0xFE, 0xFF, 0xFF, 0x87, 0xFE, 0xFF, 0xFF, 0x07};
        ...
        public static bool CharInClass(char ch, string set, ref uint[]? asciiLazyCache)
        {
            ...
            return ch < 128 ? InitializeValue(ch, set, ref asciiLazyCache) : CharInClassRecursive(ch, set, 0);
        ...
        /// </summary>
        public static bool CharInClass(char ch, string set) => CharInClassRecursive(ch, set, 0);
        ...
        public static string OneToStringClass(char c) => CharsToStringClass(stackalloc char[1]{c});
        public static unsafe string CharsToStringClass(ReadOnlySpan<char> chars)
        {
#if DEBUG
            ...
            if (rangelist != null)
            {
                ...
                if (!_negate && _subtractor is null && (_categories is null || _categories.Length == 0))
                {
                    if (rangelist.Count == 2)
                    {
                        ...
                        if (rangelist[0].First == 0 && rangelist[1].Last == LastChar && rangelist[0].Last < rangelist[1].First - 1)
                        {
        ...
        private static ReadOnlySpan<char> RangesFromProperty(string capname, bool invert, string pattern, int currentPos)
        {
            ...
            while (min != max)
            {
                ...
                else
                {
                    ...
                    return !invert ? set.AsSpan() : set[0] == '\0' ? set.AsSpan(1) : ("\0" + set).AsSpan();
                }
            }

            throw new RegexParseException(RegexParseError.UnrecognizedUnicodeProperty, currentPos, String.Format(SR.MakeException, pattern, currentPos, String.Format(SR.UnrecognizedUnicodeProperty, capname)));
        }
 ...
 set[SetStartIndex] + 1 == set[SetStartIndex + 1])
            ...
            void RenderRanges()
            {
                ...
                {
                    ch1 = set[index];
                    ch2 = index + 1 < set.Length ? (char)(set[index + 1] - 1) : LastChar;
 ...
 set[index] == 0)
        ...
        public static string DescribeChar(char ch) => ch switch
        {
            '\0' => @"\0",
            '\a' => "\\a",
            '\b' => "\\b",
            ...
            >= ' ' and <= '~' => ch.ToString(),
            _ => $"\\u{(uint)ch:X4}"};
        private static string DescribeCategory(char ch) => (short)ch switch
        {
            SpaceConst => @"\s",
            NotSpaceConst => @"\S",
