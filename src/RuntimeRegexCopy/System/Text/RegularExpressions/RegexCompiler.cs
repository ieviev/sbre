...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public abstract class RegexCompiler
    {
        ...
        /// </summary>
        public static RegexRunnerFactory? Compile(string pattern, RegexTree regexTree, RegexOptions options, bool hasTimeout) => new RegexLWCGCompiler().FactoryInstanceFromCode(pattern, regexTree, options, hasTimeout);
        ...
        /// </remarks>
        private RentedLocalBuilder RentInt32Local() => new RentedLocalBuilder(_int32LocalsPool ??= new Stack<LocalBuilder>(), _int32LocalsPool.TryPop(out LocalBuilder? iterationLocal) ? iterationLocal : DeclareInt32());
        /// <summary>Rents a ReadOnlySpan(char) local variable slot from the pool of locals.</summary>
        /// <remarks>
        /// Care must be taken to Dispose of the returned <see cref = "RentedLocalBuilder"/> when it's no longer needed,
        /// and also not to jump into the middle of a block involving a rented local from outside of that block.
        /// </remarks>
        private RentedLocalBuilder RentReadOnlySpanCharLocal() => new RentedLocalBuilder(_readOnlySpanCharLocalsPool ??= new Stack<LocalBuilder>(1), // capacity == 1 as we currently don't expect overlapping instances
 _readOnlySpanCharLocalsPool.TryPop(out LocalBuilder? iterationLocal) ? iterationLocal : DeclareReadOnlySpanChar());
        ...
        protected void EmitTryFindNextPossibleStartingPosition()
        {
            ...
            void EmitIndexOf_LeftToRight()
            {
                ...
                Ldloca(inputSpan);
                Ldloc(pos);
                if (opts.FindMode == FindNextStartingPositionMode.FixedDistanceString_LeftToRight && opts.FixedDistanceLiteral is { Distance: > 0 } literal)
                {
                ...
                Call(s_spanSliceIntMethod);
                Ldstr(opts.FindMode == FindNextStartingPositionMode.LeadingString_LeftToRight ? opts.LeadingPrefix : opts.FixedDistanceLiteral.String!);
            ...
            void EmitFixedSet_LeftToRight()
            {
                ...
                int setIndex = 0;
                bool canUseIndexOf = primarySet.Set != RegexCharClass.NotNewLineClass && (primarySet.Chars is not null || primarySet.Range is not null);
        ...
        protected void EmitTryMatchAtCurrentPosition()
        {
            ...
            void EmitConcatenation(RegexNode node, RegexNode? subsequent, bool emitLengthChecksIfRequired)
            {
                ...
                for (int i = 0; i < childCount; i++)
                {
                    ...
                    if ((node.Options & RegexOptions.RightToLeft) == 0 && emitLengthChecksIfRequired && node.TryGetJoinableLengthCheckChildRange(i, out int requiredLength, out int exclusiveEnd))
                    {
            ...
            void EmitSingleCharLoop(RegexNode node, RegexNode? subsequent = null, bool emitLengthChecksIfRequired = true)
            {
                ...
                if (!rtl && node.N > 1 && subsequent?.FindStartingLiteralNode()is RegexNode literal && CanEmitIndexOf(literal, out int literalLength))
                {
            ...
            void EmitSingleCharLazy(RegexNode node, RegexNode? subsequent = null, bool emitLengthChecksIfRequired = true)
            {
                ...
                if (!rtl && iterationCount is null && node.Kind is RegexNodeKind.Notonelazy && subsequent?.FindStartingLiteral(4)is RegexNode.StartingLiteralData literal && // 5 == max optimized by IndexOfAny, and we need to reserve 1 for node.Ch
 !literal.Negated && // not negated; can't search for both the node.Ch and a negated subsequent char with an IndexOf* method
 (literal.String is not null || literal.SetChars is not null || literal.Range.LowInclusive == literal.Range.HighInclusive || (literal.Range.LowInclusive <= node.Ch && node.Ch <= literal.Range.HighInclusive))) // for ranges, only allow when the range overlaps with the target, since there's no accelerated way to search for the union
                {
                    // e.g. "<[^>]*?>"
                ...
                else if (!rtl && iterationCount is null && node.Kind is RegexNodeKind.Setlazy && node.Str == RegexCharClass.AnyClass && subsequent?.FindStartingLiteralNode()is RegexNode literal2 && CanEmitIndexOf(literal2, out _))
                {
                ...
                if (minIterations == maxIterations)
                {
                    switch (minIterations)
                    {
                        ...
                        case 1:
                            ...
                            EmitNode(child);
                            return;
                        case> 1when !analysis.MayBacktrack(child):
        ...
        protected void EmitScan(RegexOptions options, DynamicMethod tryFindNextStartingPositionMethod, DynamicMethod tryMatchAtCurrentPositionMethod)
        {
            ...
            else if (_regexTree.FindOptimizations.FindMode is FindNextStartingPositionMode.LeadingAnchor_LeftToRight_Beginning or FindNextStartingPositionMode.LeadingAnchor_LeftToRight_Start or FindNextStartingPositionMode.LeadingAnchor_RightToLeft_Start or FindNextStartingPositionMode.LeadingAnchor_RightToLeft_End)
            {
        ...
        private void EmitMatchCharacterClass(string charClass)
        {
            ...
            if (RegexCharClass.TryGetDoubleRange(charClass, out (char LowInclusive, char HighInclusive) rangeLower, out (char LowInclusive, char HighInclusive) rangeUpper) && char.IsAsciiLetter(rangeUpper.LowInclusive) && char.IsAsciiLetter(rangeUpper.HighInclusive) && (rangeLower.LowInclusive | 0x20) == rangeUpper.LowInclusive && (rangeLower.HighInclusive | 0x20) == rangeUpper.HighInclusive)
            {
