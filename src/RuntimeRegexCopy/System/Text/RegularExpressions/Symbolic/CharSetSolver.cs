...
namespace System.Text.RuntimeRegexCopy.Symbolic
{
    ...
    /// </summary>
    public sealed class CharSetSolver : ISolver<BDD>
    {
        ...
        public BDD NonAscii => s_nonAscii ?? Interlocked.CompareExchange(ref s_nonAscii, CreateBDDFromRange('\x80', '\uFFFF'), null) ?? s_nonAscii;
        ...
        public BDD CreateBDDFromChar(char c)
        {
            ...
            if (c < (uint)ascii.Length)
            {
                ...
                return ascii[c] ?? Interlocked.CompareExchange(ref ascii[c], CreateBdd(c), null) ?? ascii[c]!;
            }
            ...
            BDD CreateBdd(ushort c)
            {
                ...
                for (int k = 0; k < 16; k++)
                {
                    bdd = (c & (1 << k)) != 0 ? GetOrCreateBDD(k, bdd, BDD.False) : GetOrCreateBDD(k, BDD.False, bdd);
                }
        ...
        public BDD CreateBDDFromRange(char lower, char upper)
        {
            ...
            return upper < lower ? Empty : upper == lower ? CreateBDDFromChar(lower) : CreateBDDFromRangeImpl(lower, upper, MaxBit);
            BDD CreateBDDFromRangeImpl(uint lower, uint upper, int maxBit)
            {
                ...
                {
                    return upper == 0 ? GetOrCreateBDD(maxBit, Empty, Full) : // lower must also be 0
