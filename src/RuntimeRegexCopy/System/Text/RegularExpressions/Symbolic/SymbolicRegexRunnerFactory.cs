...
namespace System.Text.RuntimeRegexCopy.Symbolic
{
    ...
    public sealed class SymbolicRegexRunnerFactory : RegexRunnerFactory
    {
        ...
        public SymbolicRegexRunnerFactory(RegexTree regexTree, RegexOptions options, TimeSpan matchTimeout)
        {
            ...
            if (threshold != int.MaxValue)
            {
                ...
                if (size > threshold)
                {
                    throw new NotSupportedException(String.Format(SR.NotSupported_NonBacktrackingUnsafeSize, size, threshold));
                }
            }
