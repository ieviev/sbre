...
{
    public sealed partial class SymbolicRegexMatcher<TSet>
    {
        /// <summary>
        ...
        private int[] CreateNewNfaTransition(int nfaStateId, int mintermId, int nfaOffset)
        {
            ...
            lock (this)
            {
                ...
                if (targets is null)
                {
                    ...
                    TSet minterm = GetMintermFromId(mintermId);
                    uint nextCharKind = GetPositionKind(mintermId);
                    SymbolicRegexNode<TSet>? targetNode = coreTargetId > 0 ? GetState(coreTargetId).Node : coreState.Next(_builder, minterm, nextCharKind);
                    List<int> targetsList = new();
                    ForEachNfaState(targetNode, nextCharKind, targetsList, static (int nfaId, List<int> targetsList) => targetsList.Add(nfaId));
        ...
        private (int, DerivativeEffect[])[] CreateNewCapturingTransition(int nfaStateId, int mintermId, int offset)
        {
            lock (this)
            {
                ...
                if (targets is null)
                {
                    ...
                    {
                        ForEachNfaState(entry.Node, nextCharKind, (targetsList, entry.Effects), static (int nfaId, (List<(int, DerivativeEffect[])> Targets, DerivativeEffect[] Effects) args) => args.Targets.Add((nfaId, args.Effects)));
                    }
