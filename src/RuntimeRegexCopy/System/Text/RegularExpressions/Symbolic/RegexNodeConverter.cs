...
namespace System.Text.RuntimeRegexCopy.Symbolic
{
    ...
    public sealed class RegexNodeConverter
    {
        ...
        public SymbolicRegexNode<BDD> ConvertToSymbolicRegexNode(RegexNode root)
        {
            ...
            while (stack.TryPop(out (RegexNode Node, DoublyLinkedList<SymbolicRegexNode<BDD>> Result, DoublyLinkedList<SymbolicRegexNode<BDD>>[]? ChildResults) popped))
            {
                ...
                if (childResults is null || childResults[0] is null)
                {
                    ...
                    switch (node.Kind)
                    {
                        ...
                        // Unsupported
                        default:
                            throw new NotSupportedException(String.Format(SR.NotSupported_NonBacktrackingConflictingExpression, node.Kind switch
                            {
                    ...
                    switch (node.Kind)
                    {
                        ...
                        case RegexNodeKind.Alternate:
                        {
                            ...
                            for (int i = childResults.Length - 1; i >= 0; i--)
                            {
                                ...
                                SymbolicRegexNode<BDD> elem = childResult.Count == 1 ? childResult.FirstElement : _builder.CreateConcatAlreadyReversed(childResult);
                                if (elem.IsNothing(_builder._solver))
                                {
                                    continue;
                                }

                                or = elem.IsAnyStar(_builder._solver) ? elem : // .* is the absorbing element
 SymbolicRegexNode<BDD>.CreateAlternate(_builder, elem, or);
                        ...
                        case RegexNodeKind.Lazyloop:
                        {
                            ...
                            SymbolicRegexNode<BDD> body = childResult.Count == 1 ? childResult.FirstElement : _builder.CreateConcatAlreadyReversed(childResult);
            ...
            return rootResult.Count == 1 ? rootResult.FirstElement : _builder.CreateConcatAlreadyReversed(rootResult);
        ...
        public BDD CreateBDDFromSetString(string set)
        {
            ...
            BDD Compute(string set)
            {
                ...
                BDD result = conditions.Count == 0 ? (negate ? charSetSolver.Empty : charSetSolver.Full) : (negate ? charSetSolver.And(CollectionsMarshal.AsSpan(conditions)) : charSetSolver.Or(CollectionsMarshal.AsSpan(conditions)));
                ...
                BDD MapCategoryCodeSetToCondition(HashSet<UnicodeCategory> catCodes)
                {
                    ...
                    BDD? result = null;
                    if (catCodes.Contains(UnicodeCategory.UppercaseLetter) && catCodes.Contains(UnicodeCategory.LowercaseLetter) && catCodes.Contains(UnicodeCategory.TitlecaseLetter) && catCodes.Contains(UnicodeCategory.ModifierLetter) && catCodes.Contains(UnicodeCategory.OtherLetter) && catCodes.Contains(UnicodeCategory.NonSpacingMark) && catCodes.Contains(UnicodeCategory.DecimalDigitNumber) && catCodes.Contains(UnicodeCategory.ConnectorPunctuation))
                    {
                ...
                BDD MapCategoryCodeToCondition(UnicodeCategory code)
                {
                    ...
                    return code == (UnicodeCategory)(RegexCharClass.SpaceConst - 1) ? UnicodeCategoryConditions.WhiteSpace : UnicodeCategoryConditions.GetCategory(code);
                }
            }
        }
    }
