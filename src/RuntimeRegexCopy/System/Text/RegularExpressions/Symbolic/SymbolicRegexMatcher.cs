...
namespace System.Text.RegularExpressions.Symbolic
{
    ...
    public abstract class SymbolicRegexMatcher
    {
#if DEBUG
    ...
    public sealed partial class SymbolicRegexMatcher<TSet> : SymbolicRegexMatcher where TSet : IComparable<TSet>, IEquatable<TSet>
    {
        ...
        public bool HasSubcaptures => _capsize > 1;
        /// <remarks>
        ...
        public static SymbolicRegexMatcher<TSet> Create(int captureCount, RegexFindOptimizations findOptimizations, SymbolicRegexBuilder<BDD> bddBuilder, SymbolicRegexNode<BDD> rootBddNode, ISolver<TSet> solver, TimeSpan matchTimeout)
        {
            ...
            _wordLetterForBoundariesSet = solver.ConvertFromBDD(bddBuilder._wordLetterForBoundariesSet, charSetSolver), _newLineSet = solver.ConvertFromBDD(bddBuilder._newLineSet, charSetSolver)};
        ...
        private SymbolicRegexMatcher(SymbolicRegexBuilder<TSet> builder, SymbolicRegexNode<TSet> rootNode, int captureCount, RegexFindOptimizations findOptimizations, TimeSpan matchTimeout)
        {
            ...
            if (findOptimizations.IsUseful && findOptimizations.LeadingAnchor is not RegexNodeKind.Beginning)
            {
        ...
        /// </summary>
        public PerThreadData CreatePerThreadData() => new PerThreadData(_capsize);
        ...
        /// </summary>
        public TSet GetMintermFromId(int mintermId)
        {
        ...
        private uint GetCharKind<TInputReader>(ReadOnlySpan<char> input, int i)
            where TInputReader : struct, IInputReader => !_pattern._info.ContainsSomeAnchor ? CharKind.General : // The previous character kind is irrelevant when anchors are not used.
 GetPositionKind(TInputReader.GetPositionId(this, input, i));
        ...
        public SymbolicMatch FindMatch(RegexRunnerMode mode, ReadOnlySpan<char> input, int startat, PerThreadData perThreadData)
        {
            ...
            else
            {
                ...
                matchStart = matchEnd < startat ? startat : (_pattern._info.ContainsLineAnchor, _pattern._info.ContainsSomeAnchor) switch
                {
            ...
            {
                Registers endRegisters = _pattern._info.ContainsLineAnchor ? FindSubcaptures<FullInputReader>(input, matchStart, matchEnd, perThreadData) : FindSubcaptures<NoZAnchorInputReader>(input, matchStart, matchEnd, perThreadData);
        ...
        private int FindEndPosition<TInputReader, TFindOptimizationsHandler, TNullabilityHandler>(ReadOnlySpan<char> input, int pos, long timeoutOccursAt, RegexRunnerMode mode, out int initialStatePos, out int matchLength, PerThreadData perThreadData)
            where TInputReader : struct, IInputReader where TFindOptimizationsHandler : struct, IInitialStateHandler where TNullabilityHandler : struct, INullabilityHandler
        {
            initialStatePos = pos;
            ...
            while (true)
            {
                ...
                int innerLoopLength = _checkTimeout && input.Length - pos > CharsPerTimeoutCheck ? pos + CharsPerTimeoutCheck : input.Length;
                bool done = currentState.NfaState is not null ? FindEndPositionDeltas<NfaStateHandler, TInputReader, TFindOptimizationsHandler, TNullabilityHandler>(input, innerLoopLength, mode, ref pos, ref currentState, ref endPos, ref endStateId, ref initialStatePos, ref initialStatePosCandidate) : FindEndPositionDeltas<DfaStateHandler, TInputReader, TFindOptimizationsHandler, TNullabilityHandler>(input, innerLoopLength, mode, ref pos, ref currentState, ref endPos, ref endStateId, ref initialStatePos, ref initialStatePosCandidate);
        ...
        private bool FindEndPositionDeltas<TStateHandler, TInputReader, TFindOptimizationsHandler, TNullabilityHandler>(ReadOnlySpan<char> input, int length, RegexRunnerMode mode, ref int posRef, ref CurrentState state, ref int endPosRef, ref int endStateIdRef, ref int initialStatePosRef, ref int initialStatePosCandidateRef)
            where TStateHandler : struct, IStateHandler where TInputReader : struct, IInputReader where TFindOptimizationsHandler : struct, IInitialStateHandler where TNullabilityHandler : struct, INullabilityHandler
        {
        ...
        private int FindStartPosition<TInputReader, TNullabilityHandler>(ReadOnlySpan<char> input, int i, int matchStartBoundary, PerThreadData perThreadData)
            where TInputReader : struct, IInputReader where TNullabilityHandler : struct, INullabilityHandler
        {
            ...
            while (true)
            {
                ...
                bool done = currentState.NfaState is not null ? FindStartPositionDeltas<NfaStateHandler, TInputReader, TNullabilityHandler>(input, ref i, matchStartBoundary, ref currentState, ref lastStart) : FindStartPositionDeltas<DfaStateHandler, TInputReader, TNullabilityHandler>(input, ref i, matchStartBoundary, ref currentState, ref lastStart);
        ...
        private bool FindStartPositionDeltas<TStateHandler, TInputReader, TNullabilityHandler>(ReadOnlySpan<char> input, ref int i, int startThreshold, ref CurrentState state, ref int lastStart)
            where TStateHandler : struct, IStateHandler where TInputReader : struct, IInputReader where TNullabilityHandler : struct, INullabilityHandler
        {
            ...
            where TInputReader : struct, IInputReader
        {
            ...
            current.Clear();
            next.Clear();
            ForEachNfaState(initialState.Node, initialState.PrevCharKind, (current, initialRegisters), static (int nfaId, (SparseIntMap<Registers> Current, Registers InitialRegisters) args) => args.Current.Add(nfaId, args.InitialRegisters.Clone()));
            while ((uint)i < (uint)iEnd)
            {
                ...
                foreach ((int sourceId, Registers sourceRegisters)in current.Values)
                {
                    ...
                    (int, DerivativeEffect[])[] transitions = _capturingNfaDelta[offset] ?? CreateNewCapturingTransition(sourceId, mintermId, offset);
            ...
            foreach (var(endStateId, endRegisters)in current.Values)
            {
                ...
                if (endState.IsNullableFor(GetCharKind<TInputReader>(input, iEnd)))
                {
                    ...
                    endState.Node.ApplyEffects((effect, args) => args.Registers.ApplyEffect(effect, args.Pos), CharKind.Context(endState.PrevCharKind, GetCharKind<TInputReader>(input, iEnd)), (Registers: endRegisters, Pos: iEnd));
                    return endRegisters;
                }
            }
        ...
        /// </summary>
        public sealed class PerThreadData
        {
        ...
        public sealed class NfaMatchingState
        {
            ...
            public void InitializeFrom(SymbolicRegexMatcher<TSet> matcher, MatchingState<TSet> dfaMatchingState)
            {
                ...
                matcher.ForEachNfaState(dfaMatchingState.Node, dfaMatchingState.PrevCharKind, NfaStateSet, static (int nfaId, SparseIntMap<int> nfaStateSet) => nfaStateSet.Add(nfaId, out _));
            }
        }
        ...
        private readonly struct DfaStateHandler : IStateHandler
        {
            ...
            /// </summary>
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static StateFlags GetStateFlags(SymbolicRegexMatcher<TSet> matcher, in CurrentState state) => matcher._stateFlagsArray[state.DfaStateId];
        }
            ...
            public static bool TryTakeTransition(SymbolicRegexMatcher<TSet> matcher, ref CurrentState state, int mintermId)
            {
                ...
                else
                {
                    ...
                    foreach (ref KeyValuePair<int, int> sourceState in CollectionsMarshal.AsSpan(sourceStates.Values))
                    {
                        ...
                        if (flags.SimulatesBacktracking() && (flags.IsNullable() || (flags.CanBeNullable() && matcher.GetState(coreStateId).IsNullableFor(nextCharKind))))
                        {
                            break;
                        }
                    }
        ...
        {
            public static int GetPositionId(SymbolicRegexMatcher<TSet> matcher, ReadOnlySpan<char> input, int pos) => (uint)pos >= (uint)input.Length ? -1 : matcher._mintermClassifier.GetMintermID(input[pos]);
        }
