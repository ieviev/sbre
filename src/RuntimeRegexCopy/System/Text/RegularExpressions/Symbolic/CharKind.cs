...
namespace System.Text.RuntimeRegexCopy.Symbolic
{
    public static class CharKind
    {
        ...
        public static uint Prev(uint context) => context & 0x7;
        /// <summary>Gets the next character kind from a context</summary>
        public static uint Next(uint context) => context >> 3;
        /// <summary>Encodes the pair (prevKind, nextKind) using 6 bits</summary>
        public static uint Context(uint prevKind, uint nextKind) => (nextKind << 3) | prevKind;
        ...
        public static string DescribePrev(uint i) => i switch
        {
            BeginningEnd => @"\A",
            Newline => @"\n",
            NewLineS => @"\A\n",
        ...
        public static bool IsValidCharKind(uint charKind) => charKind < CharKindCount;
    }
}