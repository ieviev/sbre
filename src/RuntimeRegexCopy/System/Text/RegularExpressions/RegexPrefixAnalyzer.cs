...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public static class RegexPrefixAnalyzer
    {
        ...
        public static List<RegexFindOptimizations.FixedDistanceSet>? FindFixedDistanceSets(RegexNode root, bool thorough)
        {
            ...
            if (results.Count == 0)
            {
                if (FindFirstCharClass(root)is not string charClass || charClass == RegexCharClass.AnyClass) // weed out match-all, same as above
                {
                    return null;
                }
        ...
        public static void SortFixedDistanceSetsByQuality(List<RegexFindOptimizations.FixedDistanceSet> results) => // Finally, try to move the "best" results to be earlier.  "best" here are ones we're able to search
 // for the fastest and that have the best chance of matching as few false positives as possible.
        results.Sort((s1, s2) =>
        {
            ...
            {
                return GetRangeLength(s1.Range.GetValueOrDefault()).CompareTo(GetRangeLength(s2.Range.GetValueOrDefault()));
                static int GetRangeLength((char LowInclusive, char HighInclusive, bool Negated) range)
                {
                    ...
                    return range.Negated ? char.MaxValue + 1 - length : length;
                }
            }
            ...
            static bool? TryFindFirstCharClass(RegexNode node, ref RegexCharClass? cc)
            {
                ...
                switch (node.Kind)
                {
                    ...
                    case RegexNodeKind.Set or RegexNodeKind.Setloop or RegexNodeKind.Setlazy or RegexNodeKind.Setloopatomic:
                    {
                        ...
                        return !setSuccess ? false : node.Kind is RegexNodeKind.Set || node.M > 0 ? true : null;
                    }
        ...
        public static RegexNodeKind FindLeadingAnchor(RegexNode node) => FindLeadingOrTrailingAnchor(node, leading: true);
        /// <summary>Computes the leading anchor of a node.</summary>
        public static RegexNodeKind FindTrailingAnchor(RegexNode node) => FindLeadingOrTrailingAnchor(node, leading: false);
