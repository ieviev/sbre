...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public sealed class RegexNode
    {
        ...
        /// </remarks>
        public RegexNode FinalOptimize()
        {
            ...
            while (true)
            {
                switch (node.Kind)
                {
                    ...
                    case RegexNodeKind.Concatenate:
                        ...
                        if ((existingChild.Kind is RegexNodeKind.Alternate or RegexNodeKind.BackreferenceConditional or RegexNodeKind.ExpressionConditional or RegexNodeKind.Loop or RegexNodeKind.Lazyloop) && (node.Parent is null || node.Parent.Kind != RegexNodeKind.Atomic)) // validate grandparent isn't atomic
                        {
        ...
        public RegexNode Reduce()
        {
            ...
            return Kind switch
            {
                RegexNodeKind.Conjunction => ReduceConjunction(),
        ...
        {
            Debug.Assert(Kind is RegexNodeKind.Alternate or RegexNodeKind.Conjunction or RegexNodeKind.Concatenate);
        ...
        /// <summary>
        /// TODO: optimize a conjunction
        /// </summary>
        private RegexNode ReduceConjunction()
        {
            Debug.Assert(Kind == RegexNodeKind.Conjunction);
            switch (ChildCount())
            {
                case 0:
                    return new RegexNode(RegexNodeKind.Nothing, Options);
                case 1:
                    return Child(0);
                default:
                    // todo: implement optimizations for negations
                    for (var i = 0; i < ChildCount(); i++)
                    {
                        if (Child(i).Options.HasFlag((RegexOptions.Negated))) return this;
                    } 
            ...
            static RegexNode ExtractCommonPrefixOneNotoneSet(RegexNode alternation)
            {
                ...
                for (int startingIndex = 0; startingIndex < children.Count - 1; startingIndex++)
                {
                    ...
                    for (; endingIndex < children.Count; endingIndex++)
                    {
                        ...
                        if (required.Kind != other.Kind || required.Options != other.Options || required.M != other.M || required.N != other.N || required.Ch != other.Ch || required.Str != other.Str)
                        {
                            break;
                        }
                    }
                ...
                for (int startingIndex = 0; startingIndex < children.Count - 1; startingIndex++)
                {
                    ...
                    for (; endingIndex < children.Count; endingIndex++)
                    {
                        ...
                        else
                        {
                            ...
                            int c = 0;
                            while (c < minLength && startingSpan[c] == startingNode.Str[c])
                                c++;
                            if (c == 0)
                            {
                                break;
                    ...
                    var prefix = startingSpan.Length == 1 ? new RegexNode(RegexNodeKind.One, startingNodeOptions, startingSpan[0]) : new RegexNode(RegexNodeKind.Multi, startingNodeOptions, startingSpan.ToString());
        ...
        private RegexNode ReduceConcatenation()
        {
            ...
            switch (ChildCount())
            {
                ...
                case 1:
                    // NEGATION, keep
                    if ((Options & RegexOptions.Negated) != 0)
                    {
                        Child(0).Options |= RegexOptions.Negated;
                    } 
                    return Child(0);
            }
        ...
        private void ReduceConcatenationWithAdjacentStrings()
        {
            ...
            // if (Children. .Options.HasFlag(RegexOptions.Negated)) return;
            Debug.Assert(Kind == RegexNodeKind.Concatenate);
            ...
            // todo: figure out what to do with negations
            for (i = 0, j = 0; i < children.Count; i++, j++)
            {
                if (children[i].Options.HasFlag((RegexOptions.Negated))) return;
            }
            ...
            for (i = 0, j = 0; i < children.Count; i++, j++)
            {
                ...
                if (at.Kind == RegexNodeKind.Concatenate && ((at.Options & RegexOptions.RightToLeft) == (Options & RegexOptions.RightToLeft)))
                {
                    if (at.Children is List<RegexNode> atChildren)
                    {
                        for (int k = 0; k < atChildren.Count; k++)
                ...
                else if (at.Kind is RegexNodeKind.Multi or RegexNodeKind.One)
                {
                    ...
                    // TODO: figure out what to do with negations in concats
                    {
            ...
            while (next < children.Count)
            {
                ...
                if (currentNode.Options == nextNode.Options)
                {
                    static bool CanCombineCounts(int nodeMin, int nodeMax, int nextMin, int nextMax)
                    {
                        ...
                        if (nodeMin == int.MaxValue || nextMin == int.MaxValue || (uint)nodeMin + (uint)nextMin >= int.MaxValue)
                        {
                            return false;
                        }

                        // Similar overflow / go infinite check for max (which can be infinite).
                        if (nodeMax != int.MaxValue && nextMax != int.MaxValue && (uint)nodeMax + (uint)nextMax >= int.MaxValue)
                        {
                            return false;
                        }
                    ...
                    switch (currentNode.Kind)
                    {
                        ...
                        case RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic or RegexNodeKind.Setlazy when nextNode.Kind == currentNode.Kind && currentNode.Str == nextNode.Str:
                            if (nextNode.M > 0 && currentNode.Kind is RegexNodeKind.Oneloopatomic or RegexNodeKind.Notoneloopatomic or RegexNodeKind.Setloopatomic)
                            {
        ...
        public int? ComputeMaxLength()
        {
            ...
            switch (Kind)
            {
                ...
                case RegexNodeKind.Multi:
                    // Every character in the string needs to match.
                    return Str!.Length;
                case RegexNodeKind.Notonelazy or RegexNodeKind.Notoneloop or RegexNodeKind.Notoneloopatomic or RegexNodeKind.Onelazy or RegexNodeKind.Oneloop or RegexNodeKind.Oneloopatomic or RegexNodeKind.Setlazy or RegexNodeKind.Setloop or RegexNodeKind.Setloopatomic:
                ...
                // The maximum length of either child branch, as long as they both have one.. The condition for an expression conditional is a zero-width assertion.
                {
                    ...
                    return Child(i).ComputeMaxLength()is int yes && Child(i + 1).ComputeMaxLength()is int no ? Math.Max(yes, no) : null;
                }
        ...
        public bool TryGetOrdinalCaseInsensitiveString(int childIndex, int exclusiveChildBound, out int nodesConsumed, [NotNullWhen(true)] out string? caseInsensitiveString)
        {
            ...
            for (; i < exclusiveChildBound; i++)
            {
                ...
                if (child.Kind is RegexNodeKind.One)
                {
                    ...
                    if (child.Ch >= 128 || RegexCharClass.ParticipatesInCaseConversion(child.Ch))
                    {
                        break;
                    }
                ...
                else if (child.Kind is RegexNodeKind.Multi)
                {
                    ...
                    if (!RegexCharClass.IsAscii(child.Str.AsSpan()) || RegexCharClass.ParticipatesInCaseConversion(child.Str.AsSpan()))
                    {
                        break;
                    }
                ...
                else if (child.Kind is RegexNodeKind.Set || (child.Kind is RegexNodeKind.Setloop or RegexNodeKind.Setlazy or RegexNodeKind.Setloopatomic && child.M == child.N))
                {
                    // In particular we want to look for sets that contain only the upper and lowercase variant
                    // of the same ASCII letter.
                    if (RegexCharClass.IsNegated(child.Str!) || RegexCharClass.GetSetChars(child.Str!, twoChars) != 2 || twoChars[0] >= 128 || twoChars[1] >= 128 || twoChars[0] == twoChars[1] || !char.IsLetter(twoChars[0]) || !char.IsLetter(twoChars[1]) || ((twoChars[0] | 0x20) != (twoChars[1] | 0x20)))
                    {
                        break;
                    }
        ...
        public bool TryGetJoinableLengthCheckChildRange(int childIndex, out int requiredLength, out int exclusiveEnd)
        {
            ...
            static bool CanJoinLengthCheck(RegexNode node) => node.Kind switch
            {
                ...
                RegexNodeKind.Oneloop or RegexNodeKind.Onelazy or RegexNodeKind.Oneloopatomic or RegexNodeKind.Notoneloop or RegexNodeKind.Notonelazy or RegexNodeKind.Notoneloopatomic or RegexNodeKind.Setloop or RegexNodeKind.Setlazy or RegexNodeKind.Setloopatomic when node.M == node.N => true,
                _ => false,
            };
        ...
        public bool SupportsCompilation([NotNullWhen(false)] out string? reason)
        {
            if ((Options & RegexOptions.NonBacktracking) != 0)
            {
        ...
        private string Describe()
        {
            ...
            // NEGATION:
            if ((Options & RegexOptions.Negated) != 0) sb.Append("-NEG");
