...
using System.Text.RuntimeRegexCopy;
...
namespace System.Text.RuntimeRegexCopy
{
    ...
    public sealed class RegexInterpreterFactory : RegexRunnerFactory
    {
        ...
        protected internal override RegexRunner CreateInstance() => // Create a new interpreter instance.
            ...
            else
            {
                while (c-- != 0)
                {
                    ...
                    if (backreferenceChar != currentChar)
                    {
                        ...
                        if (!RegexCaseEquivalences.TryFindCaseEquivalencesForCharWithIBehavior(backreferenceChar, _culture, ref _caseBehavior, out ReadOnlySpan<char> equivalences) || equivalences.IndexOf(inputSpan[pos]) < 0)
                        {
