        ...
        private void EmitFragment(RegexNodeKind nodeType, RegexNode node, int curIndex)
        {
            ...
            switch (nodeType)
            {
                ...
                case RegexNodeKind.Onelazy:
                    if (node.M > 0)
                    {
                        Emit(((node.Kind is RegexNodeKind.Oneloop or RegexNodeKind.Oneloopatomic or RegexNodeKind.Onelazy) ? RegexOpcode.Onerep : RegexOpcode.Notonerep) | bits, node.Ch, node.M);
                    }
